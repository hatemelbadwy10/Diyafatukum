...
extension StringExtension on String? {
  ...
  bool validateSaPhoneNumber() => hasMatch(this, Patterns.saPhoneNumber);

  bool validateEgPhoneNumber() => hasMatch(this, Patterns.egPhoneNumber);
  ...
  String get stripHtmlTags {
    final document = html_parser.parse(validate());
    return document.body?.text ?? validate();
  }
  ...
  DateTime toDateTime({String format = 'dd/MM/yyyy'}) {
    ...
    if (isEmptyOrNull) {
      ...
      return DateTime.tryParse(this!) ?? dateFormat.tryParse(this!) ?? DateTime.now();
    }
  }

  TimeOfDay toTimeOfDay({String format = 'HH:mm a'}) {
    DateFormat dateFormat = DateFormat(format);
    if (isEmptyOrNull) {
      return TimeOfDay.now();
    } else {
      try {
        return TimeOfDay.fromDateTime(dateFormat.parse(this!));
      } catch (e) {
        return TimeOfDay.now();
      }
    }
  ...
  bool checkFormat({required String format}) {
    try {
  ...
  Future<bool> openUrl() async {
    ...
    try {
      ...
      Toaster.showToast(LocaleKeys.error_invalid_url.tr());
      return false;
    }
  }
  ...
  Future<bool> call() async {
    ...
    if (await canLaunchUrl(uri)) {
      ...
      Toaster.showToast(LocaleKeys.error_invalid_phone.tr());
      return false;
    }
  }
  ...
  Future<bool> launchWhatsApp() async {
    final uri = Uri.parse('https://api.whatsapp.com/send?phone=$this');
    try {
      if (this!.startsWith('+')) {
        // If the phone number starts with '+', we assume it's in international format
        return await _launchWhatsApp(uri);
      } else {
        // If the phone number does not start with '+', we add it
        final formattedNumber = '+966$this'; // Assuming Saudi Arabia country code
        return await _launchWhatsApp(Uri.parse('https://api.whatsapp.com/send?phone=$formattedNumber'));
      }
    } catch (e) {
      log('Error launching WhatsApp: $e', name: 'launch');
      Toaster.showToast(LocaleKeys.error_invalid_phone.tr());
      return false;
    }
  }
  ...
  Future<bool> _launchWhatsApp(Uri uri) async {
    return await launchUrl(uri, mode: LaunchMode.externalApplication).then((value) {
      log('launching whatsapp $uri status is $value', name: 'launch');
      return value;
    });
  }
  ...
  Future<bool> sendEmail() async {
    ...
    if (await canLaunchUrl(uri)) {
      ...
      Toaster.showToast(LocaleKeys.error_invalid_email.tr());
      return false;
    }
  }
  ...
  bool get isImage => validate().validateImage();

  bool get isSvg => validate().endsWith('.svg');

  String get filePathFromUrl {
    final uri = Uri.tryParse(validate());
    if (uri == null || uri.path.isEmpty) return '';
    return uri.path.startsWith('/') ? uri.path.substring(1) : uri.path;
  }

  String get neglectStartingZero {
    if (isEmptyOrNull) return '';
    return this!.replaceFirst(RegExp(r'^0+'), '');
  }

  Widget buildNetworkAsset({
    double? height,
    double? width,
    ...
    if (isImage) {
      return CustomImage(width: width, height: height ?? 24, imageUrl: validate(), fit: fit, errorWidget: errorWidget);
    } else if (isSvg) {
...
class Patterns {
  ...
  static String saPhoneNumber = r'^5[0-9]{8}$';
  static String egPhoneNumber = r'^1[0125][0-9]{8}$';
