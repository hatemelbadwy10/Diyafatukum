...
extension NumExt on num? {
  ...
  EdgeInsetsDirectional get edgeInsetsOnlyEnd => EdgeInsetsDirectional.only(end: validate().toDouble());

  EdgeInsetsDirectional get edgeInsetsOnlyStart => EdgeInsetsDirectional.only(start: validate().toDouble());
  ...
  EdgeInsets get edgeInsetsVerticalWithBottomNavBar =>
      EdgeInsets.only(top: validate().toDouble(), bottom: AppSize.bottomNavBarHeight + validate().toDouble());
  ...
  String formattedPrice({String? currency, int decimalDigits = 0}) {
    ...
    currency ??= String.fromCharCode(0xE900);

    if (validate() == 0) {
      decimalDigits = 0;
    }

    // add space after currency or before it based on locale direction
    return NumberFormat.currency(
      name: currency,
      locale: locale,
      decimalDigits: decimalDigits,
      customPattern: '#,###.## $currency',
    ).format(validate());
  }
}
