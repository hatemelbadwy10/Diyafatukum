...
extension WidgetExtension on Widget {
  ...
  Widget setMainContainer({double? padding}) {
    ...
    return setInkContainerToView(padding: padding ?? 16, radius: AppSize.mainRadius, color: context.surfaceColor);
  }
        ...
        ? Column(
          ...
          children: [
            Row(
              ...
              children: [
                titleWidget ?? Text(title ?? '', style: titleStyle ?? context.bodyLarge.bold.s14),
  ...
  Widget withSeeAllTitle({String? title, void Function()? onTap}) {
    ...
    return setTitle(
      ...
      titleStyle: context.bodyLarge.semiBold.s16,
      titleIcon: const Text('See all').clickable(onTap: () => onTap?.call(), style: context.titleLarge.semiBold.s12),
    );
  }
  ...
  Widget setBullet({Color? bulletColor, double size = 4, double indent = 4}) {
    return Row(
      ...
      children: [CircleAvatar(radius: size, backgroundColor: bulletColor), indent.gap, flexible()],
    );
  }
  ...
  ClipRRect clipRRectOnlyWithBorder({
    ...
    return ClipRRect(
      ...
      clipBehavior: Clip.antiAliasWithSaveLayer,
      child: Container(
        decoration: BoxDecoration(border: Border.all(color: borderColor ?? Colors.transparent, width: borderWidth)),
        child: this,
      ),
    );
  }
  ...
  RefreshIndicator makeRefreshable(Future<void> Function() onRefresh, {EdgeInsetsGeometry? padding}) {
    return RefreshIndicator.adaptive(
      ...
      child: ListView(
        ...
        padding: padding ?? EdgeInsets.symmetric(horizontal: AppSize.screenPadding, vertical: 16),
        children: [this],
      ),
    );
  }
  ...
  Widget withShimmer({
    ...
    final context = rootNavigatorKey.currentContext!;
    baseColor = baseColor ?? context.customColorScheme.greySwatch[100]!;
    highlightColor = highlightColor ?? context.customColorScheme.greySwatch[200]!;
    if (!buildWhen) return this;
    return Shimmer.fromColors(
      period: duration,
      direction: context.textDirection == TextDirection.ltr ? ShimmerDirection.ltr : ShimmerDirection.rtl,
      baseColor: baseColor,
  ...
  Widget toBottomNavBar({double? bottom, Color? color, Color? borderColor}) {
    final context = rootNavigatorKey.currentContext!;
    return Column(mainAxisSize: MainAxisSize.min, children: [this])
        .withSafeArea(minimum: AppSize.screenPadding.edgeInsetsHorizontal.copyWith(bottom: bottom ?? 16))
        .paddingTop(8)
        .setContainerToView(color: color ?? context.scaffoldBackgroundColor, borderColor: borderColor);
  }
...
extension LayoutExtensions on Widget {
  ...
  Widget withListView({
    ...
    return ListView(
      ...
      padding: padding ?? EdgeInsets.all(AppSize.screenPadding).copyWith(bottom: kBottomNavigationBarHeight),
      physics: physics,
      children: [this],
    );
  }
          ...
          child: AnimatedDefaultTextStyle(
            ...
            style:
                ...
                context.displayLarge.copyWith(
                  ...
                  color: value ? context.primaryColor.withValues(alpha: 0.5) : context.primaryColor,
                ),
