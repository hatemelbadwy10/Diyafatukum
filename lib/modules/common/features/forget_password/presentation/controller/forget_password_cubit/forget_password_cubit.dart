...
class ForgetPasswordCubit extends Cubit<ForgetPasswordState> {
  ...
  ForgetPasswordCubit(this._repository)
    : super(ForgetPasswordState(status: CubitStatus.initial()));

  Future<void> forgetPassword(String value) async {
    final identifier = FlavorConfig.isProvider
        ? '+966${value.trim().neglectStartingZero}'
        : value.trim();
    ...
    response.fold(
      ...
      (_) =>
          emit(state.copyWith(status: CubitStatus.success(data: identifier))),
    );
  }
}
